...
class SuperPointFrontend(object):
    ...
    def process_pts(self, semi, coarse_desc, sampled=2000):
        ...
        dense = dense / (np.sum(dense, axis=0) + 0.00001)  # Shonwd sum to 1.
        ...
        pts[2, :] = heatmap[xs, ys]
        pts, _ = self.nms_fast(
            pts, self.h, self.w, dist_thresh=self.nms_dist
        )  # Apply NMS.
        ...
        if pts.shape[1] == 0:
            ...
            samp_pts = pts[:2, :].copy()
            samp_pts[0, :] = (samp_pts[0, :] / (float(self.w) / 2.0)) - 1.0
            samp_pts[1, :] = (samp_pts[1, :] / (float(self.h) / 2.0)) - 1.0
