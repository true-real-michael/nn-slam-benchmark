...
def convert_model(model_name: str, resize: int, output_dir: Path, dataset_path: Optional[Path] = None) -> bool:
    """
    ...
    if resize not in config["resize_values"]:
        logger.warning(f"Resize {resize} not in supported values for {model_name}: {config['resize_values']}")
        return False
    ...
    try:
        ...
        model.export_rknn(
            output_path, 
            quantization_dataset=dataset_path
        )
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="weights/rknn", 
    ...
    parser.add_argument("--models", nargs="+", default=list(MODEL_CONFIGS.keys()),
                        choices=list(MODEL_CONFIGS.keys()), help="Models to convert")
    parser.add_argument("--resize", type=int, nargs="+", help="Specific resize values to use")
    ...
    for model_name in args.models:
        ...
        resize_values = args.resize if args.resize else MODEL_CONFIGS[model_name]["resize_values"]
        
        for resize in resize_values:
            if model_name == "sela" and resize != 224:
                logger.warning(f"Sela only supports 224x224 input. Skipping resize={resize}")
                continue
    ...
    failed = any(status == "Failed" for model_results in results.values() 
